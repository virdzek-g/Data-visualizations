...
PlotCopyRatiosWithModeledSegments = function(denoised_copy_ratios_df, modeled_segments_df, contig_names, contig_starts, point_size=0.2) {
   ...
   for (s in 1:nrow(modeled_segments_df)) {
       ...
       denoised_copy_ratios[denoised_copy_ratios < 0] <- 0
       denoised_copy_ratios[denoised_copy_ratios > 4] <- 4
       copy_ratio_median <- 2^modeled_segments_df[s, "LOG2_COPY_RATIO_POSTERIOR_50"]

		# Choose color based on amplification/deletion
		segment_color <- if (copy_ratio_median > 1.1) {
		  "red"
		} else if (copy_ratio_median < 0.9) {
		  "blue"
		} else {
		  "#787575"
		}

points(x = genomic_coordinates, y = denoised_copy_ratios, col = segment_color, pch = 20, cex = 0.07)
